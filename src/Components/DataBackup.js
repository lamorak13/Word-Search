...
function Data() {
  ...
  const [words, setWords] = useState(["SEX", "ABC"]);
  const [orientations, setOrientations] = useState(['horizontal', 'vertical', 'diagonalUp', 'diagonalDown']);
  ...
  return (
    <>
      <Nav words={words} solvePuzzle={solvePuzzle} clearGrid={clearGrid} wordFind={wordFind} setSidebarActive={setSidebarActive} sidebarActive={sidebarActive}  />
      <Grid grid={grid} size={size} />
      <Sidebar open={sidebarActive} size={size} setSize={setSize} setGrid={setGrid} backwards={backwards} setBackwards={setBackwards} orientations={orientations} setOrientations={setOrientations} possibleChars={possibleChars} setPossibleChars={setPossibleChars} words={words} setWords={setWords}></Sidebar>
    </>
  );
